...
async def test_recycle_all_waste_already_processed(mock_update_storage: AsyncMock,
    ...
    Проверяется, что второй вызов возвращает сообщение о том, что все отходы уже переработаны.
...
async def test_recycle_no_storage_available(mock_update_storage: AsyncMock,
                                            ...
                                            db_session: AsyncSession) -> None:
    """
    Функция создает организацию и одно хранилище, но оно полностью заполнено. Затем проверяется,
    что запрос на перераспределение отходов возвращает ошибку о недоступности хранилищ.
